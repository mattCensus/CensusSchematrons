...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
   ...
   <!-- Checking the alternate title -->
    ...
    <sch:pattern id="browseGraphicCheck2">
        <sch:rule context="/GPM/Identification_Information[1]/Browse_Graphic">
           ...
           -->
            ...
            <sch:assert test="not(contains(./Browse_Graphic_File_Name,'&amp;VERSION'))">>GPM_NGDAChecksForSeriesInfoFiles.sch err#10: Make sure that there is an
                amp; after the ampersand for the VERSION parameter.</sch:assert>
            <sch:assert test="not(contains(./Browse_Graphic_File_Name,'&amp;LAYERS'))">>GPM_NGDAChecksForSeriesInfoFiles.sch err#11: Make sure that there is an
                amp; after the ampersand for the LAYERS parameter.</sch:assert>
            <sch:assert test="not(contains(./Browse_Graphic_File_Name,'&amp;&amp;STYLES'))">GPM_NGDAChecksForSeriesInfoFiles.sch err#12: Make sure that there is an
                amp; after the ampersand for the STYLES parameter. Also, remove the double ampersand.</sch:assert>
            <sch:assert test="not(contains(./Browse_Graphic_File_Name,'&amp;STYLES'))">>GPM_NGDAChecksForSeriesInfoFiles.sch err#13: Make sure that there is an
                amp; after the ampersand for the STYLES parameter.</sch:assert>
            <sch:assert test="not(contains(./Browse_Graphic_File_Name,'&amp;FORMAT'))">>GPM_NGDAChecksForSeriesInfoFiles.sch err#14: Make sure that there is an
                amp; after the ampersand for the FORMAT parameter.</sch:assert>
            <sch:assert test="not(contains(./Browse_Graphic_File_Name,'&amp;BGCOLOR'))">>GPM_NGDAChecksForSeriesInfoFiles.sch err#15: Make sure that there is an
                amp; after the ampersand for the BGCOLOR parameter.</sch:assert>
            <sch:assert test="not(contains(./Browse_Graphic_File_Name,'&amp;TRANSPARENT'))">>GPM_NGDAChecksForSeriesInfoFiles.sch err#16: Make sure that there is an
                amp; after the ampersand for the TRANSPARENT parameter.</sch:assert>
            <sch:assert test="not(contains(./Browse_Graphic_File_Name,'&amp;CRS'))">>GPM_NGDAChecksForSeriesInfoFiles.sch err#17: Make sure that there is an
                amp; after the ampersand for the CRS parameter.</sch:assert>
            <sch:assert test="not(contains(./Browse_Graphic_File_Name,'&amp;BBOX'))">NGDAChecksForNationalFiles.sch err#18: Make sure that there is an
                amp; after the ampersand for the BBOX parameter.</sch:assert>
            <sch:assert test="not(contains(./Browse_Graphic_File_Name,'&amp;WIDTH'))">NGDAChecksForNationalFiles.sch err#19: Make sure that there is an
                amp; after the ampersand for the WIDTH parameter.</sch:assert>
            <sch:assert test="not(contains(./Browse_Graphic_File_Name,'&amp;HEIGHT'))">NGDAChecksForNationalFiles.sch err#20: Make sure that there is an
    ...
    <sch:pattern>
        ...
        <sch:rule context="/GPM/FGDC_Required[1]/NGDA_Info[1]">
            <sch:assert test="count(NGDA_Keyword)=4">GPM_NGDAChecksForSeriesInfoFiles.sch err#21: There are supposed to be 4 keywords</sch:assert>
    ...
    <sch:pattern id="WMSandRestCheck">
        ...
        <sch:rule context="/GPM/Distribution_Information[1]/Standard_Order_Process[1]">
            ...
            <sch:assert test="count(gmd:transferOptions[1])=1">GPM_NGDAChecksForSeriesInfoFiles.sch err#26: The download zip file URL is missing!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!</sch:assert>
            <sch:assert test="count(gmd:transferOptions[2])=1">GPM_NGDAChecksForSeriesInfoFiles.sch err#29: The EAcontentInfo URL is missing!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!</sch:assert>
            <sch:assert test="count(gmd:transferOptions[3])=1">GPM_NGDAChecksForSeriesInfoFiles.sch err#29: The Technical Documentation URL is missing!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!</sch:assert>
        </sch:rule>
    </sch:pattern>
    ...
    <sch:pattern id='otherRestrictionInMD_LegalConstraints'>
        ...
        <sch:rule context="//Constraints[1]">
            <sch:report test="/GPM/Identification_Information[1]/Constraints[1]/Access_Constraints[1]='otherRestrictions' and not(gmd:otherConstraints)">GPM_NGDAChecksForSeriesInfoFiles.sch #67: If gmd:accessConstraints has a gmd:MD_RestrictionCode with a value of 'otherRestrictions' then gmd: otherConstraints must be documented.</sch:report>
