...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
    ...
    <sch:pattern>
        ...
        <sch:rule context="/gmi:MI_Metadata/gmd:dateStamp[1]/gco:Date[1]">
            ...
            <sch:assert test="contains(.,'2020')">ISO_SeriesInfoISOSchematron.sch ERR #1: The year mustbe 2020.</sch:assert>
        </sch:rule>
    </sch:pattern>
    ...
    <sch:pattern>
        <sch:rule context="/gmi:MI_Metadata/gmd:distributionInfo[1]/gmd:MD_Distribution[1]/gmd:transferOptions[1]/gmd:MD_DigitalTransferOptions[1]/gmd:onLine[1]/gmd:CI_OnlineResource[1]/gmd:linkage[1]/gmd:URL[1]">
            <sch:assert test="contains(.,'https://www.census.gov/geographies/mapping-files/time-series/geo/cartographic-boundary.html')">ISO_SeriesInfoISOSchematron.sch ERR #2: The online linkage must be https://www.census.gov/geographies/mapping-files/time-series/geo/cartographic-boundary.html</sch:assert>
        </sch:rule>
    </sch:pattern>
    ...
    <sch:pattern>
        <sch:rule context="/gmi:MI_Metadata/gmd:distributionInfo[1]/gmd:MD_Distribution[1]/gmd:transferOptions[2]/gmd:MD_DigitalTransferOptions[1]/gmd:onLine[1]/gmd:CI_OnlineResource[1]/gmd:linkage[1]">
            ...
            <sch:assert test="contains(.,'https://meta.geo.census.gov/data/existing/decennial/GEO/CPMB/boundary')">ISO_SeriesInfoISOSchematron.sch ERR #3:The EA file mustcontaoin  https://meta.geo.census.gov/data/existing/decennial/GEO/CPMB/boundary</sch:assert>
        </sch:rule>
    </sch:pattern>
    ...
    <sch:pattern>
     ...
     <sch:assert test="$KMLKeyword = $KMLTitle"> Both the title and the keywords must have KML Title<sch:value-of select="$KMLTitle"/> keyword:<sch:value-of select="$KMLKeyword"/></sch:assert>   -->
            <sch:assert test="contains(.,'SHP') or contains(.,'KML')">ISO_SeriesInfoISOSchematron.sch ERR #4: The Keywords must contain either Shapefile<sch:value-of select="$SHPTitle"/> or KML<sch:value-of select="$KMLTitle"/></sch:assert>
        </sch:rule>
    ...
    <sch:pattern>
        <sch:rule context="/gmi:MI_Metadata/gmd:identificationInfo[1]/gmd:MD_DataIdentification[1]/gmd:citation[1]/gmd:CI_Citation[1]/gmd:title[1]/gco:CharacterString[1]">
            <sch:assert test="contains(.,'Shapefile') or contains(.,'KML')">ISO_SeriesInfoISOSchematron.sch ERR #5: The Title must contain either Shapefile:<sch:value-of select="$SHPTitle"/> or KML:<sch:value-of select="$KMLTitle"/> </sch:assert>
        </sch:rule>
    </sch:pattern>
    ...
    <sch:pattern>
        ...
        <sch:rule context="//gmd:EX_GeographicBoundingBox">      
            <sch:assert test="not(gmd:westBoundLongitude/gco:Decimal &lt; -180)">ISO_SeriesInfoISOSchematron.sch ERR #6: West Bounding Longitude must be -180 or greater</sch:assert>
            <sch:assert test="not(gmd:westBoundLongitude/gco:Decimal &gt; 180)">ISO_SeriesInfoISOSchematron.sch ERR #7: West Bounding Longitude must be 180 or less</sch:assert>
            <sch:assert test="not(gmd:eastBoundLongitude/gco:Decimal &lt; -180)">ISO_SeriesInfoISOSchematron.sch ERR #8: eastBoundLongitude must be -180 or greater</sch:assert>
            <sch:assert test="not(gmd:eastBoundLongitude/gco:Decimal &lt; -180)">ISO_SeriesInfoISOSchematron.sch ERR #9: eastBoundLongitude must be -180 or greater</sch:assert>
            <sch:assert test="not(gmd:southBoundLatitude/gco:Decimal &lt; -90)">ISO_SeriesInfoISOSchematron.sch ERR #10: southBoundLatitude must be -90 or greater</sch:assert>
            <sch:assert test="not(gmd:southBoundLatitude/gco:Decimal &gt; 90)">ISO_SeriesInfoISOSchematron.sch ERR #11: southBoundLatitude must be 90 or less</sch:assert>
            <sch:assert test="not(gmd:northBoundLatitude/gco:Decimal &lt; -90)">ISO_SeriesInfoISOSchematron.sch ERR #12: northBoundLatitude must be -90 or greater</sch:assert>
            <sch:assert test="not(gmd:northBoundLatitude/gco:Decimal &gt; 90)">ISO_SeriesInfoISOSchematron.sch ERR #13: northBoundLatitude must be 90 or less</sch:assert>
            <!--  -->
