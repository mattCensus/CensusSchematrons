...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
    ...
    <sch:pattern>
     ...
     <sch:assert test="$KMLKeyword = $KMLTitle"> Both the title and the keywords must have KML Title<sch:value-of select="$KMLTitle"/> keyword:<sch:value-of select="$KMLKeyword"/></sch:assert>   -->
            <sch:assert test="contains(.,'SHP') or contains(.,'KML') or contains(.,'geodatabase')">ERR #3: The Keywords must contain either Shapefile<sch:value-of select="$SHPTitle"/>, KML<sch:value-of select="$KMLTitle"/> or goedatabase <sch:value-of select="$GDBKeyword"/></sch:assert>
        </sch:rule>
   ...
   <sch:pattern>
       <sch:rule context="/gmi:MI_Metadata/gmd:identificationInfo[1]/gmd:MD_DataIdentification[1]/gmd:citation[1]/gmd:CI_Citation[1]/gmd:title[1]/gco:CharacterString[1]">
           <sch:assert test="contains(.,'Shapefile') or contains(.,'KML')  or contains(.,'geodatabase')">ERR #4: The Title must contain either Shapefile:<sch:value-of select="$SHPTitle"/> or KML:<sch:value-of select="$KMLTitle"/> </sch:assert>
       </sch:rule>
   </sch:pattern>
